...
async function initPokemonData(num) {
  ...
  try {
    ...
    if (i <= 1) {
      i = i; // Access the 'i' value from Alpine.js state
      console.log("i < 1: ", i);
      ...
      i = i - 1; // Access the 'i' value from Alpine.js state
      console.log("i > 1: ", i);
      response = await axios.get(`../pokemon_data/pokemon_${i}_data.json`);
      data = response.data;
      // Set the fetched data in Alpine.js state
    ...
    // prevPokemon = this.prevPokemon;
    // prevPokemonName = prevPokemon.name;
    ...
    // console.log("nextPokemon:", nextPokemonName);
